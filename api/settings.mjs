...
export default async function handler(request, response) {
  ...
  try {
    ...
    await sql`UPDATE app_state SET settings = CAST(${JSON.stringify({ language: settings.language, font: settings.font, customFeatures: settings.customFeatures !== false, hidePersonalContentWhenDisabled: settings.hidePersonalContentWhenDisabled !== false, collectionFeatures: settings.collectionFeatures !== false })} AS jsonb), updated_at = NOW() WHERE id = 1`
