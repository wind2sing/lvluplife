...
- 创建、编辑、复制和封印个人任务，可配置说明、等级、经验、属性奖励、循环周期、完成提示、所需精力与情境标签。
...
- 私人收藏馆包含称号、徽章、头像框、营地主题与纪念物，支持搜索、状态筛选、多种排序、真实解锁进度和跨设备装备同步。
- 设置页可以整体关闭私人收藏馆；关闭时隐藏入口并暂停装扮效果，但不会删除解锁进度和装备选择。
- 已解锁收藏可以生成仅下载到本机的 PNG 私人纪念卡；收藏品不提供属性或经验优势。
- 个人统计支持 7 天、30 天和一年周期，展示经验、完成次数、属性成长、分类分布与行动时段。
...
- 任务等级提供小幅经验修正，不会压过任务稀有度与精力需求。
- 所需精力与重复周期自动推导“轻松一胜、支线任务、进阶挑战、史诗任务”稀有度。
- 属性预算按分类对应的主属性和副属性分配；轻松任务通常只奖励主属性。

个人任务默认采用自动计算。填写分类、重复周期、主要属性和所需精力后，界面会实时显示稀有度、经验与属性点。高级用户仍可切换到手动调整模式。系统不再猜测任务需要多少分钟。
...
| `/settings` | 界面、字体、个人创作与收藏馆模块开关 |
| `/about` | 玩法原则、成长机制、数据隐私与项目来源说明 |
