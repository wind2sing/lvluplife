...
type SaveState = {
  ...
  notificationTrackingVersion: number
...
function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const bootstrapChallengeMap = new Map([...challengeCatalog, ...initialSave.customChallenges].map((item) => [item.id, item]))
        ...
        if (initialSave.notificationTrackingVersion < 1) {
          const migrationChallenges = [...challengeCatalog, ...initialSave.customChallenges]
          const migrationChallengeMap = new Map(migrationChallenges.map((item) => [item.id, item]))
          const migrationCompleted = initialSave.completions.map((completion) => ({ completion, challenge: migrationChallengeMap.get(completion.challengeId) })).filter((item): item is { completion: Completion; challenge: Challenge } => Boolean(item.challenge))
          const migrationXp = migrationCompleted.reduce((sum, item) => sum + getEarnedReward(item.completion, item.challenge).xp, 0)
          const migrationLevel = getLevel(migrationXp).level
          const migrationStats = migrationCompleted.reduce((result, item) => { getEarnedReward(item.completion, item.challenge).stats.forEach((stat) => { result[stat.key] += stat.points }); return result }, { STR: 0, CUL: 0, ENV: 0, CHA: 0, TAL: 0, INT: 0 } as Record<StatKey, number>)
          const migrationDiscovery = buildDiscoveryState(migrationChallenges, initialSave, migrationLevel).discovered
          const migrationCollection = buildCollection(migrationCompleted, initialSave.plans, migrationStats, getStreak(initialSave.completions), migrationXp)
          initialSave = {
            ...initialSave,
            notificationTrackingVersion: 1,
            seenChallengeIds: migrationChallenges.filter((item) => migrationDiscovery.has(item.id) && item.level <= migrationLevel).map((item) => item.id),
            seenCollectionIds: migrationCollection.filter((item) => item.unlocked).map((item) => item.id),
          }
        }
        localStorage.removeItem(STORAGE_KEY)
        setChallenges(challengeCatalog)
        setSave(initialSave)
  ...
  const newChallengeIds = useMemo(() => new Set(allChallenges.filter((item) => discoveredIds.has(item.id) && item.level <= level.level && !save.hiddenIds.includes(item.id) && !save.seenChallengeIds.includes(item.id)).map((item) => item.id)), [allChallenges, discoveredIds, level.level, save.hiddenIds, save.seenChallengeIds])
  const newCollectionIds = useMemo(() => new Set(collectionItems.filter((item) => item.unlocked && !save.seenCollectionIds.includes(item.id)).map((item) => item.id)), [collectionItems, save.seenCollectionIds])
  ...
  const recommendationPool = unlockedChallenges.filter((item) => !save.hiddenIds.includes(item.id) && (!catalogCategories.has(item.category) || save.activeIds.includes(item.id) || save.favoriteIds.includes(item.id)))
  ...
  function saveCustomChallenge(challenge: Challenge) {
    setSave((current) => ({
      ...
      seenChallengeIds: [...new Set([...current.seenChallengeIds, challenge.id])],
    }))
    setCustomEditor(null)
  ...
  function createPlan(input: { title: string; description: string; kind: QuestPlan['kind']; category: string; stat: StatKey; steps: string[] }) {
    ...
    const stepChallenges = input.steps.map((stepTitle, index): Challenge => {
      ...
      const reward = calculateReward({ tier: index === input.steps.length - 1 ? 3 : 2, energyDemand: index === input.steps.length - 1 ? 'high' : 'normal', primaryStat: input.stat, secondaryStat })
      return {
    ...
    setSave((current) => ({ ...current, plans: [plan, ...current.plans], customChallenges: [...stepChallenges, ...current.customChallenges], seenChallengeIds: [...new Set([...current.seenChallengeIds, ...stepChallenges.map((item) => item.id)])] }))
    setPlanEditor(false)
    navigate('plans')
  }
  ...
  function duplicateCustomChallenge(challenge: Challenge) {
    ...
    setSave((current) => ({ ...current, customChallenges: [copy, ...current.customChallenges], seenChallengeIds: [...new Set([...current.seenChallengeIds, copy.id])] }))
    openChallenge(copy)
  }
  ...
  function openCollectionItem(item: CollectionItem) {
    ...
    if (newCollectionIds.has(item.id)) setSave((current) => ({ ...current, seenCollectionIds: [...new Set([...current.seenCollectionIds, item.id])] }))
    setView('collection')
  ...
  const mainContent = (() => {
    ...
    if (view === 'collection' && settings.collectionFeatures) return <CollectionGallery equipped={save.cosmetics} items={collectionItems} newIds={newCollectionIds} onEquip={equipCollectionItem} onOpen={openCollectionItem} />
    ...
    if (view === 'explore') {
      return (
        <ExploreView
          ...
          level={level.level}
          newChallengeIds={newChallengeIds}
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'sidebar--open' : ''}`}>
        ...
        <nav className="main-nav" aria-label={text('主导航', 'Main navigation')}>
          ...
          {settings.collectionFeatures && <NavButton active={view === 'collection'} badge={newCollectionIds.size} icon={Gem} label={text('私人收藏馆', 'Collection')} onClick={() => navigate('collection')} />}
          <NavButton active={view === 'explore'} badge={newChallengeIds.size} icon={Compass} label={text('任务公会', 'Quest Guild')} onClick={() => navigate('explore')} />
...
function NavButton({ active, badge, icon: Icon, label, onClick }: { active: boolean; badge?: number; icon: LucideIcon; label: string; onClick: () => void }) {
  return <button className={active ? 'active' : ''} onClick={onClick}><Icon size={19} /><span>{label}</span>{Boolean(badge) && <b>{badge && badge > 99 ? '99+' : badge}</b>}</button>
}
...
function CollectionGallery({ equipped, items, newIds, onEquip, onOpen }: { equipped: CosmeticState; items: CollectionItem[]; newIds: Set<string>; onEquip: (item: CollectionItem) => void; onOpen: (item: CollectionItem) => void }) {
  const { language, text } = useLanguage()
...
function ExploreView({ activeIds, category, categoryDiscovery, completions, favoriteIds, fogPreviewChallenges, hiddenIds, hiddenLockedCount, level, newChallengeIds, onCategory, onComplete, onCreate, onFavorite, onOpen, onSeal, onShowSealed, onStart, search, sealedCount, setSearch, showSealed, totalChallenges, unlockedTotal, visibleChallenges }: QuestActions & {
  ...
  return (
    <>
      ...
      {categoryDiscovery?.mode !== 'catalog' && visibleChallenges.length > 80 && <p className="result-note">{text('当前显示前 80 项结果，请使用搜索或分类继续缩小范围。', 'Showing the first 80 results. Search or filter to narrow the list.')}</p>}
    </>
  )
}
...
function AboutView() {
  ...
  return (
    <>
      ...
      <section className="about-facts" aria-label={text('项目概览', 'Project overview')}>
        <div><strong>538</strong><span>{text('项原版挑战', 'legacy quests')}</span></div><div><strong>18</strong><span>{text('个生活分类', 'life categories')}</span></div><div><strong>6</strong><span>{text('项现实属性', 'real-life stats')}</span></div><div><strong>5</strong><span>{text('种重复周期', 'repeat cadences')}</span></div><div><strong>3</strong><span>{text('个每日候选', 'daily picks')}</span></div><div><strong>{text('私人', 'Private')}</strong><span>{text('单人云存档', 'solo cloud save')}</span></div>
      </section>
      ...
      <section className="about-rule-panel">
        <div className="about-section-heading"><div><p className="eyebrow">{text('迷雾与探索', 'Fog and discovery')}</p><h2>{text('大多数任务不会按总等级一次性全部解锁', 'Most quests do not all unlock from global level')}</h2></div><p>{text('探索型分类必须同时通过“总等级资格”和“分类探索阶段”两道门槛。经典书单是图鉴型例外：150 本全部为等级 1，并从一开始完整开放。', 'Exploration categories require both global-level eligibility and category discovery. The classic reading list is a catalog exception: all 150 books are level 1 and open from the start.')}</p></div>
        <div className="about-rule-table" role="table" aria-label={text('分类探索规则', 'Category discovery rules')}>
          <div className="about-rule-row about-rule-row--head" role="row"><span>{text('机制', 'Rule')}</span><span>{text('作用', 'Purpose')}</span><span>{text('精确规则', 'Exact behavior')}</span></div>
          <div className="about-rule-row" role="row"><strong>{text('总等级', 'Global level')}</strong><span>{text('领取资格', 'Eligibility')}</span><code>{text('任务等级 ≤ 当前等级', 'quest level ≤ current level')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('初始发现', 'Starter reveal')}</strong><span>{text('新分类起点', 'New category start')}</span><code>{text('最多 4 项', 'up to 4 quests')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('不同完成', 'Unique completions')}</strong><span>{text('扩大候选', 'Expand choices')}</span><code>{text('每项 +2 个发现名额', '+2 reveal slots each')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('封印补位', 'Sealed replacement')}</strong><span>{text('避免被卡住', 'Avoid dead ends')}</span><code>{text('每项 +1 个发现名额', '+1 reveal slot each')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('分类阶段', 'Category tiers')}</strong><span>{text('开放更深任务', 'Open deeper quests')}</span><code>{text('不同完成 1 / 3 / 10 项 → Tier 2 / 3 / 4', '1 / 3 / 10 unique completions → Tier 2 / 3 / 4')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('图鉴目录', 'Catalog category')}</strong><span>{text('完整公开', 'Fully open')}</span><code>{text('经典书单：150 项均为 Lv.1 / 终身一次', 'Classic books: all 150 are Lv.1 / lifetime')}</code></div>
          <div className="about-rule-row" role="row"><strong>{text('永久发现', 'Permanent discovery')}</strong><span>{text('保留探索成果', 'Keep exploration')}</span><code>discoveredIds</code></div>
        </div>
        <p className="about-formula"><span>{text('发现目标数', 'Reveal target')}</span><strong>{text('4 + 不同任务完成数 × 2 + 当前分类封印数', '4 + unique completions × 2 + sealed quests in category')}</strong><small>{text('实际结果仍受分类 Tier、任务总等级和分类任务总数限制，并按原挑战顺序发现。', 'The result is still capped by category tier, quest level, and category size, following original quest order.')}</small></p>
      </section>
      <div className="about-grid">
        <section className="about-card"><div className="about-card-heading"><Zap size={21} /><div><span>01</span><h2>{text('经验、等级与迷雾', 'XP, levels, and fog')}</h2></div></div><ul><li>{text('1 → 2 级需要 500 XP，之后每级需求增加 180。', 'Level 1 → 2 needs 500 XP; each next level costs 180 more.')}</li><li>{text('已发现但等级不足：可看详情，不可领取。', 'Revealed but under-level: visible, not claimable.')}</li><li>{text('未发现：不可搜索、直达或进入推荐。', 'Undiscovered: excluded from search, direct routes, and recommendations.')}</li></ul></section>
        ...
        <section className="about-card"><div className="about-card-heading"><FileText size={21} /><div><span>06</span><h2>{text('数据与附件', 'Data and attachments')}</h2></div></div><ul><li>{text('正式进度：Neon PostgreSQL。', 'Production progress: Neon PostgreSQL.')}</li><li>{text('私密附件：Vercel Blob；撤销记录时同步删除。', 'Private files: Vercel Blob, deleted when completion is undone.')}</li><li>{text('本地开发：SQLite；访问由 PERSONAL_ACCESS_KEY 保护。', 'Local development: SQLite; access is protected by PERSONAL_ACCESS_KEY.')}</li></ul></section>
      </div>
      <section className="about-notes"><div><p className="eyebrow">{text('开始前必须知道', 'Before you begin')}</p><h2>{text('可回退，不惩罚，也不偷偷改写历史', 'Reversible, non-punitive, and history-safe')}</h2></div><ul><li>{text('行动力不是生命值；耗尽不会失败或受罚。', 'Energy is not health; reaching zero causes no failure or penalty.')}</li><li>{text('撤销完成会扣回经验与属性，并删除该记录附件。', 'Undoing removes its XP, stats, and attachments.')}</li><li>{text('封印不会删除任务，功能开关也不会删除个人内容。', 'Sealing does not delete quests, and feature toggles do not delete personal content.')}</li><li>{text('每次完成保存奖励快照，未来公式变化不会改写历史奖励。', 'Each completion stores a reward snapshot, so future formulas do not rewrite history.')}</li><li>{text('自定义任务与计划步骤自动发现；旧存档里已互动的任务会自动保留。', 'Custom quests and plan steps are auto-revealed; interacted legacy quests are preserved during migration.')}</li></ul></section>
...
function CustomQuestModal({ challenge, currentLevel, onClose, onSave }: { challenge: Challenge | null; currentLevel: number; onClose: () => void; onSave: (challenge: Challenge) => void }) {
  ...
  const autoReward = useMemo(() => calculateReward({ tier: tierValue, energyDemand, primaryStat: statKey, secondaryStat }), [energyDemand, secondaryStat, statKey, tierValue])
  ...
  function submit(event: React.FormEvent) {
    ...
    const cleanPoints = Math.min(18, Math.max(1, Math.round(statPoints)))
    ...
    <div className="modal-backdrop" role="presentation" onClick={onClose}>
      <form className="custom-quest-modal" role="dialog" aria-modal="true" aria-labelledby="custom-quest-title" onClick={(event) => event.stopPropagation()} onSubmit={submit}>
        ...
        <div className="custom-form-grid">
          ...
          <label><span>{text('建议解锁等级', 'Suggested level')}</span><input type="number" min="1" max="30" value={levelValue} onChange={(event) => setLevelValue(Number(event.target.value))} /></label>
          ...
          <label className="field-wide"><span>{text('任务难度', 'Task difficulty')}</span><select value={tierValue} onChange={(event) => setTierValue(Number(event.target.value))}><option value="1">1 · 轻松一胜（10–30 分钟）</option><option value="2">2 · 支线任务（30–120 分钟）</option><option value="3">3 · 进阶挑战（半天或多阶段）</option><option value="4">4 · 史诗任务（多日或重大里程碑）</option></select></label>
          ...
          {rewardMode === 'auto' ? <div className="reward-calculator field-wide"><div><span>{text('任务档位', 'Tier')}</span><strong>{autoReward.tierName}</strong></div><div><span>{text('经验奖励', 'XP')}</span><strong>+{autoReward.xp}</strong></div>{autoReward.stats.map((stat) => <div key={stat.key}><span>{statLabels[stat.key]}</span><strong>+{stat.points}</strong></div>)}<p><Sparkles size={14} /> {text(`奖励只根据任务难度、完成当日所需精力和实际属性计算；${cadence}仅控制冷却，不再降低或抬高奖励。`, 'Rewards use task difficulty, completion energy, and actual stats. Cadence only controls cooldown.')}</p></div> : <><label><span>{text('经验奖励', 'XP reward')}</span><input type="number" min="25" max="1500" step="5" value={xp} onChange={(event) => setXp(Number(event.target.value))} /></label><label><span>{text('属性点', 'Stat points')}</span><input type="number" min="1" max="18" value={statPoints} onChange={(event) => setStatPoints(Number(event.target.value))} /></label></>}
